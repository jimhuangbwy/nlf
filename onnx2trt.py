...
def onnx2engine(
    ...
    if dynamic:
        ...
        min_shape = (1, 3, 384, 384)#(1, shape[1], 32, 32)  # minimum input shape
        opt_shape = (1, 3, 384, 384)
        max_shape = (1, 3, 384, 384)#(*shape[:2], *(int(max(2, workspace or 2) * d) for d in shape[2:]))  # max input shape
        for inp in inputs:
    ...
    print(f"{prefix} building {'INT8' if int8 else 'FP' + ('16' if half else '32')} engine as {engine_file}")
    if int8:
