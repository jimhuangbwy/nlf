...
from nlf.common.util import TRAIN, VALID, TEST
...
class LocalizerFieldJob(fleras.TrainingJob):
    def __init__(self):
        super().__init__(
            ...
            force_grad_accum=FLAGS.force_grad_accum,
    ...
    def build_data(self):
        ...
        # PARAMETRIC
        bad_paths_param = set(spu.load_pickle(f'{DATA_ROOT}/posepile_33ds_new/bad_paths_param.pkl'))
    ...
    def learning_rate_schedule(self, step):
        ...
        if step_float < n_warmup_steps:
            ...
            return tf.keras.optimizers.schedules.ExponentialDecay(
                b * FLAGS.field_lr_factor,
                decay_rate=1 / 3,
                decay_steps=n_phase1_steps,
                staircase=False,
            )(step_float)
        else:
            return tf.keras.optimizers.schedules.ExponentialDecay(
                ...
                staircase=False,
            )(step_float - n_warmup_steps - n_phase1_steps)
    ...
    def learning_rate_schedule_finetune_low(self, step):
        ...
        if step_float < n_frozen_steps:
            ...
            return tf.keras.optimizers.schedules.ExponentialDecay(
                b * FLAGS.backbone_lr_factor,
                decay_rate=1 / 3,
                decay_steps=n_phase1_steps,
                staircase=False,
            )(step_float - n_warmup_steps - n_frozen_steps)
        else:
            return tf.keras.optimizers.schedules.ExponentialDecay(
                b * tf.cast(1 / 30, tf.float32),
                decay_rate=0.3,
                decay_steps=n_phase2_steps,
                staircase=False,
            )(step_float - n_warmup_steps - n_frozen_steps - n_phase1_steps)

    def build_callbacks(self):
        cbacks = [render_callback.RenderPredictionCallback(start_step=500, interval=100)]
